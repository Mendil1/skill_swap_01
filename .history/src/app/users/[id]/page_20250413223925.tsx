...
import { BookOpen, Calendar, GraduationCap, Mail, MessageSquare, User } from "lucide-react";
              ...
              {isOwnProfile ? (
                ...
                <div className="flex flex-wrap gap-2 relative group">
                  ...
                  <div className="absolute bottom-full mb-2 right-0 opacity-0 group-hover:opacity-100 transition-opacity bg-black text-xs text-white p-2 rounded pointer-events-none whitespace-nowrap">
                    Coming soon! Connection requests will be available in the next update.
                  </div>
                </div>
              )}
            </div>
          ...
          <CardContent className="space-y-6">
            ...
            {userData.availability ? (
              ...
              <div>
                ...
                <p className="text-slate-500 italic">No availability information provided yet.</p>
              </div>
            )}
          </CardContent>
        </Card>
      ...
      <Tabs defaultValue="teaches" className="space-y-6">
        <TabsList className="bg-slate-100 p-1">
          <TabsTrigger value="teaches" className="flex items-center gap-2">
            ...
            <span>Skills {isOwnProfile ? "You" : userData.full_name.split(' ')[0]} Teach{isOwnProfile ? "" : "es"}</span>
          </TabsTrigger>
          <TabsTrigger value="learns" className="flex items-center gap-2">
            <BookOpen className="h-4 w-4 text-green-600" />
            <span>Skills {isOwnProfile ? "You" : userData.full_name.split(' ')[0]} Want{isOwnProfile ? "" : "s"} to Learn</span>
          </TabsTrigger>
        </TabsList>
          ...
          <Card>
            <CardHeader className="bg-blue-50 border-b border-blue-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle>Skills {isOwnProfile ? "You" : userData.full_name.split(' ')[0]} Can Teach</CardTitle>
                  <CardDescription>
                    These are the skills {isOwnProfile ? "you" : userData.full_name.split(' ')[0]} can share with others
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="pt-6">
              {offeredSkills.length > 0 ? (
                ...
                <div className="text-center py-8 text-slate-500 bg-slate-50 rounded-md">
                  ...
                  <p>{isOwnProfile ? "You haven't" : `${userData.full_name.split(' ')[0]} hasn't`} added any skills {isOwnProfile ? "you" : "they"} can teach yet.</p>
                  {isOwnProfile && (
                    <p className="mt-2">
                      <Link href="/profile" className="text-blue-600 hover:underline">
                        Go to your profile
                      </Link> to add skills you can teach.
                    </p>
                  )}
                </div>
              )}
          ...
          <Card>
            <CardHeader className="bg-green-50 border-b border-green-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle>Skills {isOwnProfile ? "You" : userData.full_name.split(' ')[0]} Want{isOwnProfile ? "" : "s"} to Learn</CardTitle>
                  <CardDescription>
                    These are the skills {isOwnProfile ? "you're" : `${userData.full_name.split(' ')[0]} is`} looking to learn
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="pt-6">
              {requestedSkills.length > 0 ? (
                ...
                <div className="text-center py-8 text-slate-500 bg-slate-50 rounded-md">
                  ...
                  <p>{isOwnProfile ? "You haven't" : `${userData.full_name.split(' ')[0]} hasn't`} added any skills {isOwnProfile ? "you" : "they"} want to learn yet.</p>
                  {isOwnProfile && (
                    <p className="mt-2">
                      <Link href="/profile" className="text-blue-600 hover:underline">
                        Go to your profile
                      </Link> to add skills you want to learn.
                    </p>
                  )}
                </div>
              )}
