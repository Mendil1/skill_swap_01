...
import { CheckCheck, Clock, AlertCircle, MessageSquareHeart, RefreshCw, ChevronDown } from "lucide-react";
...
export default function MessageList({
  ...
  useEffect(() => {
    ...
    const handleFocus = () => { isUserInputActive.current = true; };
    const handleBlur = () => { isUserInputActive.current = false; };
  ...
  useEffect(() => {
    ...
    const setupRealtimeSubscription = async () => {
      realtimeChannel = supabaseRef.current.channel(`messages:${conversationId}`);
    ...
    return () => {
      ...
      if (realtimeChannel) {
        supabaseRef.current.removeChannel(realtimeChannel);
      }
    };
        ...
        {Object.entries(groupedMessages).map(([date, dateMessages]) => (
          <div key={date} className="space-y-3">
            ...
            {dateMessages.map((message, index) => {
              ...
              const isRecent = new Date().getTime() - new Date(message.sent_at).getTime() < 60000;
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <div className={`relative rounded-lg py-2 px-3 text-sm hover:shadow-md transition-shadow
                      ${isCurrentUser 
