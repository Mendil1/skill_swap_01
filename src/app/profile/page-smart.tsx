...
function ProfileContent() {
  ...
  useEffect(() => {
    async function loadProfileData() {
      ...
      // If no user in auth context after loading, try to get session directly
      if (!user) {
        ...
        try {
          ...
          console.log("[Profile] Direct getSession result:", {
          ...
          // No session, try getUser to check server auth
          console.log("[Profile] No session found, checking server auth with getUser...");
          const { data: userData, error: userError } = await supabase.auth.getUser();
          console.log("[Profile] Direct getUser result:", {
          ...
          if (userData.user && !userError) {
            console.log("[Profile] Server confirmed user authentication, loading profile");
    ...
    async function loadRealUserData(authUser: AuthUser) {
      try {
        ...
        const transformedSkills: UserSkill[] = (userSkills || []).map(
          (rawSkill: {
            user_skill_id: string;
            type: "offer" | "request" | "both";
