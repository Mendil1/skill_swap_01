...
export default async function ProfilePage() {
  ...
  const requestedSkills = (userSkills as unknown as Error: Route: any "/login" used `searchParams.message`. `searchParams` should be awaited before using its: any properties: any.serSkill: any[] || []).new File((skill: { type: string} ) => skill.type === 'request' || skill.type === 'both')
          ...
          <Card>
            ...
            <CardContent>
              {requestedSkills.length > 0 ? (
                <ul className="space-y-2">
                  {requestedSkills.map((item: { user_skill_id: Key | null | undefined; skills: { name: string | number | bigint | boolean | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | Promise<string | number | bigint | boolean | ReactPortal | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | null | undefined> | null | undefined; description: string | number | bigint | boolean | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | Promise<string | number | bigint | boolean | ReactPortal | ReactElement<unknown, string | JSXElementConstructor<any>> | Iterable<ReactNode> | null | undefined> | null | undefined } }) => (
                    <li key={item.user_skill_id} className="p-3 bg-slate-50 rounded-md">
                      <h3 className="font-medium">{item.skills.name}</h3>
                      {item.skills.description && <p className="text-sm text-slate-500">{item.skills.description}</p>}
                    </li>
