      ...
      <div className="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {(sessions as Session[]).filter((s: Session) => s.status === "scheduled").length +
                (groupSessions as GroupSession[]).filter(
                  (s: GroupSession) => s.status === "scheduled"
                ).length}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {Math.round(
                ((sessions as Session[]).reduce(
                  (acc: number, s: Session) => acc + (s.duration_minutes || 60),
                  0
                ) +
                  (groupSessions as GroupSession[]).reduce(
                    (acc: number, s: GroupSession) => acc + (s.duration_minutes || 60),
                    0
                  )) /
                  60
              )}
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(groupSessions as GroupSession[]).length}</div>
