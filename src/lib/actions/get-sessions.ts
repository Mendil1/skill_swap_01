...
export async function getSessionsServerAction() {
  ...
  try {
    ...
    const { data: groupParticipants }: { data: Array<{
      group_session_id: string;
      user_id: string;
    }> | null } = groupSessions && groupSessions.length > 0 ? 
      await supabase
        ...
        .select(`
          ...
          user_id
        `)
        .in("group_session_id", groupSessions.map(gs => gs.id)) : { data: [] };
    ...
    groupParticipants?.forEach(gp => {
      ...
      const participantProfile = profileMap.get(gp.user_id) || { full_name: "Unknown User", email: "" };
    ...
    const transformedSessions = sessions?.map(session => ({
      ...
      participant: profileMap.get(session.participant_id) || { full_name: "Unknown User", email: "" }
    })) || [];

    const transformedGroupSessions = groupSessions?.map(session => ({
      id: session.id,
      topic: session.topic,
      scheduled_at: session.scheduled_at,
      duration_minutes: session.duration_minutes,
